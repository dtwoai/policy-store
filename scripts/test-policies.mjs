...
function assertOpaAvailable() {
  try {
    ...
    console.error(`Could not run '${opa}'. Install OPA (v1.x) and put it on PATH, or set OPA_BIN.`);
    console.error("See https://www.openpolicyagent.org/docs/latest/#running-opa");
    process.exit(1);
  }
}
